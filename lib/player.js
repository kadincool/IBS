...
export function movePlayer() {
  ...
  if (inputs.crouch && !pos.crouched) {
    ...
    else if (!climbMove && !sideMove && inputs.up && attemptMove(-1, 1, true)) {climbMove = true; sideMove = true} //push up and side
    else if (!climbMove && inputs.up && attemptMove(0, 2, true)) {climbMove = true}
    else if (!climbMove && !sideMove && inputs.up && attemptMove(-1, 2, true)) {climbMove = true; sideMove = true}
    else if (!sideMove && attemptMove(-1, 0, true)) {sideMove = true} //push back
    else if (!sideMove && attemptMove(-2, 0, true)) {sideMove = true}
  }
