...
export function movePlayer() { //this whole system has many flaws but frick it, we ball
  ...
  if (!inputs.crouch && pos.crouched && !attemptMove(0, 0, false)) {
    ...
    if (attemptMove((pos.left ? -1 : 1), 0, false) || attemptMove((pos.left ? -2 : 2), 0, false)) sideMove = true; // side
    if (attemptMove((pos.left ? -1 : 1), -1, false) || attemptMove((pos.left ? -1 : 1), -2, false) || attemptMove((pos.left ? -2 : 2), -1, false) || attemptMove((pos.left ? -2 : 2), -2, false)) sideMove = true; // side & down
  }
  //reset if on floor
  if (!attemptMove(0, -1, pos.crouched, pos.left, false)) {
