...
export function movePlayer() { //this whole system has many flaws but frick it, we ball
  ...
  if (!inputs.grab || inputs.sright || inputs.sleft || inputs.sup || inputs.sdown) stats.grabbed = false;
  if (inputs.sreset) {
    pos.targetX = 0;
    pos.targetY = 0;
  }
  ...
  if (!stats.switched) stats.currentInv = lib.pmod(stats.currentInv + inputs.hbnext - inputs.hbprev, stats.inventory.length);
  if (inputs.hbnext ^ inputs.hbprev) stats.switched = true; else stats.switched = false;
  if (inputs.grab && !stats.grabbed) {
    let toPlace = stats.inventory[stats.currentInv];
    stats.inventory[stats.currentInv] = terrain.getTile(pos.x + pos.targetX, pos.y + pos.targetY);
    terrain.setTile(pos.x + pos.targetX, pos.y + pos.targetY, toPlace);
    stats.grabbed = true;
  }
}
