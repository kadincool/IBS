...
export function movePlayer() {
  ...
  //climbing
  //*
  //console.log(inputs.jump, stats.energy >= 1)
  if (inputs.jump && stats.energy >= 1) { 
    if (xAdd && !sideMove && !climbMove) {
      if (attemptMove(0, 1)) {
        climbMove = true;
        stats.energy-=1;
      } else if (inputs.up && !sideMove && attemptMove((pos.left ? 1 : -1), 1)) {
        climbMove = true;
        sideMove = true;
        stats.energy-=1;
      } else {
    ...
    } else if (inputs.up && !attemptMove(0, 1, pos.crouched, pos.left, false)) {
      climbMove = true;
      stats.energy-=1;
    }
  }//*/
  /*
  if (inputs.jump && stats.energy>=1) {
    if (!sideMove && xAdd && !climbMove && !attemptMove(0, 1) && inputs.up && attemptMove((pos.left ? 1 : -1), 1)) {
      sideMove = true;
    }
    climbMove = true;
    stats.energy-=1;
