...
export async function draw() {
  ...
  // can2d.scale(0.5, 0.5);
  // can2d.translate(window.innerWidth / 2, window.innerHeight / 2);
  can2d.fillStyle = "#4bf";
  ...
  //draw the blocks
  for (let i = Math.floor(-canvas.width / lib.graphics.scale / (terrain.chunkSize * 2) + camera.x / terrain.chunkSize); i < Math.ceil(canvas.width / lib.graphics.scale / (terrain.chunkSize * 2) + camera.x / terrain.chunkSize); i++) {
    for (let j = Math.floor(-canvas.height / lib.graphics.scale / (terrain.chunkSize * 2) + camera.y / terrain.chunkSize); j < Math.ceil(canvas.height / lib.graphics.scale / (terrain.chunkSize * 2) + camera.y / terrain.chunkSize); j++) {
      terrain.drawChunk(i, j, i * terrain.chunkSize, j * terrain.chunkSize);
    }
  }
