...
export async function draw() {
  ...
  can2d.fillRect(camera.x-8, camera.y+8, Math.ceil(player.stats.maxHealth/4), 1);
  can2d.fillStyle = "red";
  can2d.fillRect(camera.x-8, camera.y+8, Math.ceil(player.stats.health/4), 1);
  can2d.fillStyle = "green";
  can2d.fillRect(camera.x-8, camera.y+7, Math.ceil(player.stats.maxEnergy/4), 1);
  can2d.fillStyle = "lime";
  can2d.fillRect(camera.x-8, camera.y+7, Math.ceil(player.stats.energy/4), 1);
  can2d.fillStyle = "black";
  can2d.fillRect(camera.x-8, camera.y+6, player.stats.inventory.length, 1);
  for (let i = 0; i < player.stats.inventory.length; i++) {
    terrain.drawTile(terrain.tiles[player.stats.inventory[i]],camera.x-8+i, camera.y+6);
  }
  terrain.drawTile({color1: "#0000", color2: "#fff", pattern: 0x000f}, camera.x-8+player.stats.currentInv, camera.y+5)
}