...
export function genChunk(chunkX, chunkY) {
  ...
  for (let i=0; i<chunkDat.length; i++) {
    ...
    let val2d = perlin2d(x, y, world.seed, 3, 5);
    ...
    if (val2d > 0.5 && y > 10) chunkDat[i] = 1;
    if (y == height) chunkDat[i] = 2;
    else if (y < height-3) chunkDat[i] = 7;
    else if (y < height) chunkDat[i] = 3;
...
export function setTile(tileX, tileY, set) {
  ...
  if (typeof set == "number") {
    ...
    chunks[getChunk(fdiv(tileX, 8), fdiv(tileY, 8))][pmod(tileX, 8) + pmod(tileY, 8) * 8] = tiles.findIndex((element) => element.name == set);
  }
}
...
export const tiles = [
  ...
  {color1: "#aaa", color2: "#b99", pattern: 0xc060, name: "rock"},
];
