...
export function getTile(tileX, tileY) {
  return tiles[chunks[getChunk(fdiv(tileX, 8), fdiv(tileY, 8))][pmod(tileX, 8) + pmod(tileY, 8) * 8]];
}

export function setTile(tileX, tileY, set) {
  chunks[getChunk(fdiv(tileX, 8), fdiv(tileY, 8))][pmod(tileX, 8) + pmod(tileY, 8) * 8] = tiles.find((element) => {element.name == set});
}
