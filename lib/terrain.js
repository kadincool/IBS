...
export function drawChunk(chunkX, chunkY, x, y, can2d, mono) {
  ...
  for (let i=0; i<data.length; i++) {
    ...
    drawTile(tiles[data[i]], x+pmod(i,8), y+fdiv(i,8), can2d, mono);
  }
}
...
export function drawTile(tile, x, y, can2d, mono) {
  ...
  if (lib.graphics.mono) {
    ...
    lib.can2d.fillRect(x*4, y*4,4,4);
  } else {
    for (let i=0; i<16; i++) {
      ...
      lib.can2d.fillRect(x*4+pmod(i,4),y*4+(fdiv(i,4)),1,1);
    }
  }
}
